...
#
# Se lo script gira su GitHub Actions questo valore viene rilevato AUTOMATICAMENTE
# e non serve toccare nulla. Modificalo solo se esegui lo script sul tuo computer.
...
def resolve_site_base() -> str:
    """Su GitHub Actions ricava l'URL di Pages da GITHUB_REPOSITORY (formato 'owner/repo')."""
...
def build_feed(episodes: list[dict]) -> str:
    ...
    cover_url = f"{BASE}/{COVER}" if COVER else ""
    ...
    for ep in episodes:
        url = f"{BASE}/{AUDIO_DIR}/{ep['filename']}"
        add('    <item>')
...
def main() -> None:
    if "TUOUSERNAME" in BASE:
        print("!! ATTENZIONE: devi sostituire TUOUSERNAME in SITE_BASE,")
        print("   oppure eseguire lo script tramite GitHub Actions (rilevamento automatico).\n")
    else:
